...
```
2) Dekodowanie Base64 -> cipher2.bin:
```bash
echo -n "Po8rckj2i+oe7YEKbwn61g==" | base64 -d > cipher2.bin
```
3) Deszyfrowanie CAMELLIA-128-ECB:
```bash
...
```
2) Generujemy klucz szyfrujący:
```bash
openssl rand -hex 24
```
3) Szyfrujemy z użyciem klucza:
```bash
...
openssl rand -hex 16
```
3) Generujemy wektor inicjalizacyjny:
```bash
openssl rand -hex 16
```
4) Szyfrujemy:
...
```
2) Dekodujemy base64 do pliku:
```bash
echo "<encrypted_b64>" | base64 -d > cipher.bin
```
3) Odszyfrujemy AES-256-CBC:
```bash
...
## Zadanie 6 - Symetryczne AES
1) Szyfrujemy słowo za pomocą algorytmu AES-256-CBC:
```bash
openssl enc -aes-256-cbc -in plaintext.txt -out encrypted.bin -k password
```
2) Deszyfrujemy za pomocą AES:
```bash
openssl enc -d -aes-256-cbc -in encrypted.bin -out decrypted.txt -k password
```

## Zadanie 7 - Asymetryczne RSA
1) Generujemy parę kluczy RSA:
```bash
openssl genpkey -algorithm RSA -out private_key.pem -pkeyopt rsa_keygen_bits:2048
openssl rsa -pubout -in private_key.pem -out public_key.pem
```
2) Wysyłamy klucze RSA na serwer:
```bash
curl -X POST 127.0.0.1:3001/submit/public -F "file=@public_key.pem"
curl -X POST 127.0.0.1:3001/submit/private -F "file=@private_key.pem"
```

## Zadanie 8 - Przechowywanie danych w pliku
1) Szyfrujemy dane:
```bash
gpg --encrypt --recipient user@example.com file.txt
```
2) Wysyłamy zaszyfrowany plik na serwer:
```bash
curl -X POST 127.0.0.1:3001/submit -F "file=@file.txt.gpg"
```

## Zadanie 9 - Łamanie haseł
1) Wysłanie requestu na endpoint:
```bash
curl 127.0.0.1:4001/hash
```
2) Łamanie hasła:
```bash
hashcat -m 0 <hash_file> /path/to/wordlist
```

## Zadanie 10 - Szyfrowanie RSA z paddingiem OAEP
1) Szyfrujemy tekst z wykorzystaniem algorytmu RSA-2048 i paddingiem OAEP:
```bash
openssl rsautl -encrypt -inkey public_key.pem -in word.txt -out encrypted.bin -oaep
```
2) Deszyfrujemy:
```bash
openssl rsautl -decrypt -inkey private_key.pem -in encrypted.bin -out decrypted.txt -oaep
```
## Zadanie 11 - Generowanie kluczy RSA 2048 bitów
1) Generujemy parę kluczy RSA:
```bash
openssl genpkey -algorithm RSA -out private_key.pem -pkeyopt rsa_keygen_bits:2048
openssl rsa -pubout -in private_key.pem -out public_key.pem
```
2) Eksportowanie klucza publicznego i prywatnego:
```bash
openssl rsa -in private_key.pem -pubout -out public_key.pem
```
3) Wysłanie kluczy do serwera:
```bash
curl -X POST 127.0.0.1:5001/submit/public -F "file=@public_key.pem"
curl -X POST 127.0.0.1:5001/submit/private -F "file=@private_key.pem"
```

## Zadanie 12 - Generowanie kluczy RSA 2048 bitów z paddingiem OAEP
1) Generujemy parę kluczy RSA:
```bash
openssl genpkey -algorithm RSA -out private_key.pem -pkeyopt rsa_keygen_bits:2048
openssl rsa -pubout -in private_key.pem -out public_key.pem
...
2) Wysłanie kluczy do serwera:
```bash
curl -X POST 127.0.0.1:5002/submit/public -F "file=@public_key.pem"
curl -X POST 127.0.0.1:5002/submit/private -F "file=@private_key.pem"
```

## Zadanie 13 - Generowanie klucza publicznego RSA
1) Generowanie klucza publicznego z prywatnego:
```bash
openssl rsa -in private_key.pem -pubout -out public_key.pem
```
2) Wysłanie klucza publicznego do serwera:
```bash
curl -X POST 127.0.0.1:5003/submit/public -F "file=@public_key.pem"
```

## Zadanie 14 - Złamanie hasła MD5
1) Pobieramy skrót MD5:
```bash
curl 127.0.0.1:5004/hash
```
2) Łamanie MD5:
```bash
hashcat -m 0 <hash_file> /path/to/wordlist.txt
```

## Zadanie 15 - Generowanie kluczy GPG i RSA
1) Generowanie kluczy GPG RSA:
```bash
gpg --gen-key
```
2) Eksportowanie kluczy:
```bash
gpg --armor --export -o public.key
gpg --armor --export-secret-keys -o private.key
```
3) Wysłanie kluczy do serwera:
```bash
curl -X POST 127.0.0.1:5005/submit/public -F "file=@public.key"
curl -X POST 127.0.0.1:5005/submit/private -F "file=@private.key"
```

## Zadanie 16 - Zaszyfrowanie pliku Ubuntu
1) Szyfrujemy plik za pomocą AES:
```bash
openssl enc -aes-128-cbc -in ubuntu.txt -out encrypted.bin -k secretkey
```
2) Deszyfrowanie:
```bash
openssl enc -d -aes-128-cbc -in encrypted.bin -out decrypted.txt -k secretkey
```

## Zadanie 17 - Generowanie kluczy RSA z domyślnymi parametrami
1) Generujemy parę kluczy RSA:
```bash
...
2) Wysłanie kluczy do serwera:
```bash
curl -X POST 127.0.0.1:5006/submit/public -F "file=@public_key.pem"
curl -X POST 127.0.0.1:5006/submit/private -F "file=@private_key.pem"
```

## Zadanie 18 - Szyfrowanie i deszyfrowanie za pomocą AES128
1) Szyfrujemy plik:
```bash
openssl enc -aes-128-cbc -in input.txt -out encrypted.txt -k secretkey
```
2) Deszyfrowanie:
```bash
openssl enc -d -aes-128-cbc -in encrypted.txt -out decrypted.txt -k secretkey
```

## Zadanie 19 - Uruchamianie serwera i wysyłanie plików
1) Uruchomienie serwera:
```bash
docker run -p 5000:5000 --name server docker.io/mazurkatarzyna/enc-server:latest
```
2) Wysłanie tekstu na serwer:
```bash
curl -X POST 127.0.0.1:5000/submit -H "Content-Type: application/json" -d '{"word":"<word>"}'
```
